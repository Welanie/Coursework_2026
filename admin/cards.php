...
<section class="form-panel">
    <h1><?= $edit ? 'Редактировать карточку' : 'Новая карточка' ?></h1>
    ...
    <form method="post">
        ...
        <div class="field">
            ...
            <select id="set_id" name="set_id" required>
                <?php foreach ($sets as $set): ?>
                    <option value="<?= h((string) $set['id']) ?>" <?= (int) ($edit['set_id'] ?? 0) === (int) $set['id'] ? 'selected' : '' ?>>
                        <?= h($set['name']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="actions">
            ...
            <?php if ($edit): ?><a class="button ghost" href="<?= h(app_url('admin/cards.php')) ?>">Отмена</a><?php endif; ?>
        </div>
    </form>
</section>
...
<section class="table-wrap" style="margin-top: 18px;">
    <table>
        ...
        <?php foreach ($cards as $card): ?>
            <tr>
                <td><?= h((string) $card['id']) ?></td>
                <td><?= render_formula($card['formula']) ?></td>
                <td><?= h($card['name']) ?></td>
                <td><?= h($card['category']) ?></td>
                <td><?= h($card['set_name']) ?></td>
                <td class="inline-actions">
                    <a href="<?= h(app_url('admin/cards.php?edit=' . $card['id'])) ?>">Изменить</a>
